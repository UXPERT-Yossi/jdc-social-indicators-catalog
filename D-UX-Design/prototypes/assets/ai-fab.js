...
(function () {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      /* FAB button — bottom-left (= inline-end in RTL) */
      .ai-fab {
        position: fixed;
        ...
        display: grid;
        place-items: center;
        box-shadow: 0 6px 20px rgba(15,23,42,.20), 0 2px 6px rgba(15,23,42,.12);
        transition: transform .2s cubic-bezier(.4,0,.2,1), box-shadow .2s, opacity .2s;
        font-family: 'Heebo', system-ui, sans-serif;
      }
      .ai-fab:hover {
        transform: translateY(-2px);
        box-shadow: 0 10px 28px rgba(15,23,42,.28), 0 3px 8px rgba(15,23,42,.16);
      }
      ...
      /* Side panel — anchored to left edge (= inline-end in RTL),
         slides in from the left. Hidden state: pushed leftward off-screen. */
      .ai-panel {
        position: fixed;
        inset-block-start: 0;
        inset-block-end: 0;
        ...
        z-index: 1001;
        background: #FFFFFF;
        border-inline-start: 1px solid #E2E8F0;  /* border on right edge of panel in RTL */
        box-shadow: -6px 0 28px rgba(15,23,42,.10);
        display: flex;
        flex-direction: column;
        transform: translateX(-100%);  /* slide leftward off-screen (physical) */
        transition: transform .35s cubic-bezier(.4,0,.2,1);
        font-family: 'Heebo', system-ui, sans-serif;
      }
      .ai-panel.open { transform: translateX(0); }

      /* Body push when panel is open — left padding pushes content right.
         In RTL, "left padding" = padding-inline-end. */
      body.ai-panel-open {
        padding-inline-end: 420px;
        transition: padding-inline-end .35s cubic-bezier(.4,0,.2,1);
      }
  ...
  function buildFab() {
    ...
    btn.appendChild(el('span', { class: 'ai-fab-pulse', 'aria-hidden': 'true' }));
    btn.appendChild(iconSparkle());
    return btn;
  }
